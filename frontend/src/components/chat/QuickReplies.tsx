        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none" style={{ scrollbarWidth: "none" }}>
          {SUGGESTIONS.map((s) => (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--surface)] border border-[var(--border)] rounded-full text-xs text-[var(--text-2)] hover:border-[var(--brand)] hover:text-[var(--brand)] hover:bg-[var(--brand-dim)] active:scale-95 transition-all shadow-sm whitespace-nowrap flex-shrink-0"
            >
              <span>{s.icon}</span>
              <span>{s.label}</span>
            </button>
